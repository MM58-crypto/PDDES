...
def ghq_view(request):
    if request.method =="POST":
        ...
        if form.is_valid():
           ...
           if score <= 11:
               result = "No significant psychological distress"
               context = {
                    'conclusion': result, 'score':score
               }
               return render(request, "Screening_pgs/ghq_result.html", context )
           elif score >= 12 and score <= 23:
               result = "May indicate mild to moderate psychological problems"
               context = {
                    'conclusion': result, 'score':score
               }
               return render(request, "Screening_pgs/ghq_result.html", context )
           elif score > 23 and score <= 36:
               result = "Significant psychological distress. Might indicate severe psychological issues"
               context = {
                    'conclusion': result, 'score':score
               }
               return render(request, "Screening_pgs/ghq_result.html", context )
        else:
            messages.error(request,"There is an issue the form is not submitted to the server")
...
def ghq_results(request):
    #
    return render(request, "Screening_pgs/ghq_result.html")
